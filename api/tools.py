...
def clean_string(title: str) -> str:
    ...
    title = re.sub(
        r"\s*\((Deluxe( (Edition|Version))?|Super Deluxe Edition|Expanded( (Edition|Version))?|Extended Edition|10( Year)? Anniversary Edition|40( Year)? Anniversary Edition|The Ultimate Collection|Re-?Master(ed)?|Special Edition|[12][0-9]{3}( Version| Remaster(ed)?| Mix)?)\)",
        "",
        title,
        flags=re.IGNORECASE,
    )
